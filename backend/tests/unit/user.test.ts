import { isStudent, hasPendingRequest, getAllClubs } from '../../src/utils/user';
...
describe('User Utils', () => {
    const mockClubs = [
        {
            id: 1,
            name: 'Chess Club',
            shortDescription: 'A club for chess enthusiasts.',
            image: 'chessclub.png',
            isCommittee: true,
        },
        {
            id: 2,
            name: 'Hiking Club',
            shortDescription: 'Exploring nature through hikes.',
            image: 'hikingclub.png',
            isCommittee: false,
        },
    ];
    ...
    describe('isStudent', () => {
        it('should return true if the user is a student', async () => {
            ...
            expect(pool.query).toHaveBeenCalledWith('SELECT studentnumber FROM users WHERE id = $1', [1]);
        });
    ...
    describe('hasPendingRequest', () => {
        ...
        it('should throw an error if the database query fails', async () => {
            ...
            await expect(hasPendingRequest(mockUserId, mockClubId)).rejects.toThrow('Failed to check pending request.');
            expect(pool.query).toHaveBeenCalledWith(
                'SELECT 1 FROM requests WHERE memberId = $1 AND clubId = $2 AND status = $3',
                [mockUserId, mockClubId, 'Pending']
            );
    ...
    describe('getAllClubs', () => {
        ...
        it('should return a list of clubs with committee status', async () => {
            ...
            expect(normalizeQuery(mockedPoolQuery.mock.calls[0][0])).toEqual(expectedQuery);
            expect(mockedPoolQuery).toHaveBeenCalledWith(expect.any(String), [mockUserId]);
        });
        ...
        it('should throw an error if the database query fails', async () => {
            ...
            await expect(getAllClubs(mockUserId)).rejects.toThrow('Failed to fetch user memberships');
            expect(normalizeQuery(mockedPoolQuery.mock.calls[0][0])).toEqual(expectedQuery);
            expect(mockedPoolQuery).toHaveBeenCalledWith(expect.any(String), [mockUserId]);
        });
    });  
