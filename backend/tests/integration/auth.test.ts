...
describe('Authentication API Integration Tests', () => {
  ...
  describe('POST /auth/login', () => {
    it('should log in successfully with valid credentials', async () => {
      mockQuery.mockResolvedValueOnce({
        rows: [
          { id: 1, email: 'test@example.com', password: 'hashed_password', isactive: true },
        ],
      });
      ...
      expect(mockSign).toHaveBeenCalledWith({ id: 1, email: 'test@example.com' }, expect.any(String), {
        expiresIn: '24h',
      });
    });
    ...
    it('should return 401 for invalid credentials', async () => {
      mockQuery.mockResolvedValueOnce({
        rows: [{ id: 1, email: 'test@example.com', password: 'hashed_password', isactive: true }],
      });
  ...
  describe('POST /auth/signup', () => {
    it('should sign up a new user and send a verification email', async () => {
      ...
      mockGenerateVerificationToken.mockReturnValueOnce('mock_verification_token');
  ...
  describe('GET /auth/verify', () => {
    it('should verify the user email', async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE users SET isActive = true WHERE id = $1'),
        [1]
      );
    });
    ...
    it('should return 400 for an invalid or expired token', async () => {
      ...
      const res = await request(app).get('/auth/verify').query({ token: 'invalid_token' });
    ...
    it('should fetch user details for an authenticated user', async () => {
      mockQuery
        .mockResolvedValueOnce({
          rows: [
            { id: 1, name: 'Test User', email: 'test@example.com', university: 'Test University', studentNumber: '12345' },
          ],
        })
        .mockResolvedValueOnce({
      ...
      expect(res.body).toMatchObject({
        ...
        isStudent: true,
        studentNumber: '12345',
        clubs: { rows: [{ id: 1, name: 'Test Club' }]},
      });
      expect(mockQuery).toHaveBeenCalledTimes(2);
    });
  });
